dataSetVersion = "2023-03-03"; // Change this when creating a new data set version. YYYY-MM-DD format.
dataSet[dataSetVersion] = {};

dataSet[dataSetVersion].options = [
  {
    name: "Filter by Series Entry",
    key: "series",
    tooltip: "Check this to restrict to certain albums.",
    checked: false,
    sub: [
      { name: "The Black Parade", key: "parade" },
      { name: "Three Cheers for Sweet Revenge", key: "revenge" },
      { name: "Danger Days", tooltip: "Danger Days: The True Lives of the Fabulous Killjoys", key: "danger" },
      { name: "Bullets", tooltip: "I Brought You My Bullets, You Brought Me Your Love", key: "bullets" },
      { name: "MCR Singles", tooltip: "Any MCR demos, songs, or singles", key: "mcr" },
      { name: "Conventional Weapons", tooltip: "Conventional Weapons 1,2,3,4,5", key: "weapons" },
      { name: "Missile Kid", tooltip: "The Mad Gear and Missile Kid", key: "missile" },
      { name: "Evening Out With Your Girlfriend", key: "girlfriend" },
      { name: "Take This to Your Grave", key: "grave" },
      { name: "Heart B-Side", tooltip: "My Heart Will Always Be the B-Side to My Tounge", key: "tounge" },
      { name: "From Under The Cork Tree", key: "cork" },
      { name: "Infinity On High", key: "infinity" },
      { name: "Folie a Deux", key: "folie" },
      { name: "PAX AM Days", key: "pax" },
      { name: "Save Rock and Roll", key: "save" },
      { name: "American Beauty/American Psycho", key: "abap" },
      { name: "MANIA", key: "mania" },
      { name: "Lake Effect Kid", key: "lake" },
      { name: "So Much (For) Stardust", key: "stardust" },
      { name: "FOB Singles", key: "fob" }
    ]
  }
];

dataSet[dataSetVersion].characterData = [
  {
    name: "The End.",
    img: "FyL1OFN.png",
    opts: {
      series: [ "parade"]
    }
  },
  {
    name: "Dead!",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "This Is How I Disappear",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "The Sharpest Lives",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "Welcome to the Black Parade",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "I Don't Love You",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "House of Wolves",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "Cancer",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "Mama",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "Sleep",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "Teenagers",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "Disenchanted",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "Famous Last Words",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "Blood",
    img: "FyL1OFN.png",
    opts: {
      series: ["parade"]
    }
  },
  {
    name: "Helena",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "Give 'Em Hell, Kid",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "To the End",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "You Know What They Do to Guys Like Us In Prison",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "I'm Not Okay (I Promise)",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "The Ghost of You",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "The Jetset Life is Gonna Kill You",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "Interlude",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "THank You for the Venom",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "Hang 'Em High",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "It's Not a Fashion Statement, It's a Fucking Deathwish",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "Cemetary Drive",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "I Never Told You What I Do for a Living",
    img: "ASOfAzW.jpeg",
    opts: {
      series: ["revenge"]
    }
  },
  {
    name: "Look Alive, Sunshine",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "Na Na Na (Na Na Na Na Na Na Na Na Na)",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "Bulletproof Heart",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "SING",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "Planetary (GO!)",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "The Only Hope For Me Is You",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "Jet-Star and the Kobra Kid",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "Party Poison",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "Save Yourself, I'll Hold Them Back",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "S/C/A/R/E/C/R/O/W",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "Summertime",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "DESTROYA",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "The Kids from Yesterday",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "Goodnite, Dr. Death",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "Vampire Money",
    img: "MXkm08P.jpeg",
    opts: {
      series: ["danger"]
    }
  },
  {
    name: "Romance",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Honey, This Mirror Isn't Big Enough For The Two of Us",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Vampires Will Never Hurt You",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Our Lady of Sorrows",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Headfirst For Halos",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Drowning Lessons",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Skylines and Turnstiles",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Early Sunsets over Monroeville",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "This Is the Best Day Ever",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Cubicles",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Demolition Lovers",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Romance",
    img: "gkEdbff.png",
    opts: {
      series: ["bullets"]
    }
  },
  {
    name: "Under Pressure",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Foundations of Decay",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "The Five of Us Are Dying",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Kill All Your Friends",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Party at the End of the World",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "My Way Home is Through You",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Not That Kind of Girl",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Emily",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "All the Angels",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Knives/Sorrow",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Surrender the Night",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["weapons"]
    }
  },
  {
    name: "Kiss the Ring",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["weapons"]
    }
  },
  {
    name: "Make Room!!!!",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["weapons"]
    }
  },
  {
    name: "The World Is Ugly",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["weapons"]
    }
  },
  {
    name: "The Light Behind Your Eyes",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["weapons"]
    }
  },
  {
    name: "AMBULANCE",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["weapons"]
    }
  },
  {
    name: "Gun.",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["weapons"]
    }
  },
  {
    name: "Boy Division",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["weapons"]
    }
  },
  {
    name: "Tomorrow's Money",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["weapons"]
    }
  },
  {
    name: "F.T.W.W.W",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["missile"]
    }
  },
  {
    name: "Mastas of Ravenkroft",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["missile"]
    }
  },
  {
    name: "Black Dragon Fighting Society",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["missile"]
    }
  },
  {
    name: "Desolation Row",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Desert Song",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Bury Me in Black",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Under Pressure",
    img: "P8FcGir.jpeg",
    opts: {
      series: ["mcr"]
    }
  },
  {
    name: "Honorable Mention",
    img: "LW7m8b2.png",
    opts: {
      series: ["girlfriend"]
    }
  },
  {
    name: "Calm Before the Storm",
    img: "LW7m8b2.png",
    opts: {
      series: ["girlfriend"]
    }
  },
  {
    name: "Switchblades and Infidelity",
    img: "LW7m8b2.png",
    opts: {
      series: ["girlfriend"]
    }
  },
  {
    name: "Pretty In Punk",
    img: "LW7m8b2.png",
    opts: {
      series: ["girlfriend"]
    }
  },
  {
    name: "Growing Up",
    img: "LW7m8b2.png",
    opts: {
      series: ["girlfriend"]
    }
  },
  {
    name: "The World's Not Waiting (For Five Tired Boys In A Broken Down Van)",
    img: "LW7m8b2.png",
    opts: {
      series: ["girlfriend"]
    }
  },
  {
    name: "Short, Fast, and Loud",
    img: "LW7m8b2.png",
    opts: {
      series: ["girlfriend"]
    }
  },
  {
    name: "Moving Pictures",
    img: "LW7m8b2.png",
    opts: {
      series: ["girlfriend"]
    }
  },
  {
    name: "Parker Lewis Can't Lose (But I'm Gonna Give It My Best Shot)",
    img: "LW7m8b2.png",
    opts: {
      series: ["girlfriend"]
    }
  },
  {
    name: "Tell That Mick He Just Made My Day",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Tell That Mick He Just Made My List of Things To Do Today",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Dead on Arrival",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Grand Theft Autumn/Where Is Your Boy Tonight",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Saturday",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Saturday",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Homesick at Space Camp",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Sending Postcards from a Plane Crash (Wish You Were Here)",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Chicago Is So Two Years Go",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "The Pros and Cons of Breathing",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Grenade Jumper",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Calm Before the Storm",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Reinventing the Wheel to Run Myself Over",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "Patron Saint of Liars and Fakes",
    img: "fIKRGqg.png",
    opts: {
      series: ["grave"]
    }
  },
  {
    name: "My Heart Is the Worst Kind of Weapon",
    img: "fIKRGqg.png",
    opts: {
      series: ["tounge"]
    }
  },
  {
    name: "It's Not A Side Effect of the Cocaine, I Am Thinking It Must Be Love",
    img: "fIKRGqg.png",
    opts: {
      series: ["tounge"]
    }
  },
  {
    name: "Nobody Puts Baby in the Corner",
    img: "fIKRGqg.png",
    opts: {
      series: ["tounge"]
    }
  },
  {
    name: "My Heart Is the Worst Kind of Weapon",
    img: "fIKRGqg.png",
    opts: {
      series: ["tounge"]
    }
  },
  {
    name: "Love Will Tear Us Apart",
    img: "fIKRGqg.png",
    opts: {
      series: ["tounge"]
    }
  },
  {
    name: "Our Lawyer Made Us Change The Name of This Song So We Wouldn't Get Sued",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "Of All The Gin Joints In the World",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "Of All The Gin Joints In the World",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "Dance, Dance",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "Sugar, We're Goin Down",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "Nobody Puts Baby in the Corner",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "I've Got A Dark Alley and a Bad Idea That Says Shut Your Mouth (Summer Love)",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "7 Minutes in Heaven (Atavan Halen)",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "Sophmore Slump or the Comeback of the Year",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "Champagne For My Real Friends, Sham Pain For My Fake Friends",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "I Slept With Someone In Fall Out Boy, And All I Got Was This Stupid Song Written About Me",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "A Little Less Sixteen Candles, A Little More Touch Me",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "Get Busy Living or Get Busy Dying (Do Your Part to Save the Scene And Stop Going To Shows)",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "XO",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "Snitches and Talkers Get Stitches And Walkers",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "The Music or The Misery",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "My Heart is The Worst Kind of Weapon",
    img: "TthwyIE.png",
    opts: {
      series: ["cork"]
    }
  },
  {
    name: "Thriller",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "Take Over, The Break's Over",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "This Ain't A Scene, It's an Arms Race",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "I'm Like A Lawyer With The Way I'm Trying To Get You Off (Me & You)",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "Hum Hallelujah",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "Golden",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "Thnks fr th Mmrs",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "Don't You Know Who I Think I Am?",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "The (After) Life Of The Party",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "The Carpal Tunnel Of Love",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "Bang The Doldrums",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "Fame > Infamy",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "You're Crashing, But You're No Wave",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "I've Got All This Ringing In My Ears, And None On My Finger",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "G.I.N.A.S.S",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "It's Hard To Say I Do, When I Don't",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "G.I.N.A.S.S",
    img: "dBxdmg3.png",
    opts: {
      series: ["infinity"]
    }
  },
  {
    name: "Disloyal Order Of Water Buffaloes",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "I Don't Care",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "She's My Winona",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "America's Suitehearts",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "Headfirst Slide Into Cooperstown",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "The (Shipped) Golden Standard",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "(Coffee's For Closers)",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "What A Catch, Donnie",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "27",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "Tiffany Blews",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "w.a.m.s",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "20 Dollar Nose Bleed",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "West Coast Smoker",
    img: "xvc2lWl.png",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "Disloyal Order Of Water Buffaloes",
    img: "D9hKx.jpeg",
    opts: {
      series: ["folie"]
    }
  },
  {
    name: "Alpha Dog",
    img: "RekBTIA.jpeg",
    opts: {
      series: ["fob"]
    }
  },
  {
    name: "From Now On We Are Enemies",
    img: "RekBTIA.jpeg",
    opts: {
      series: ["fob"]
    }
  },
  {
    name: "Growing Up",
    img: "RekBTIA.jpeg",
    opts: {
      series: ["fob"]
    }
  },
  {
    name: "We Were Doomed From The Start (The King is Dead)",
    img: "MNGKQDW.png",
    opts: {
      series: ["pax"]
    }
  },
  {
    name: "Art of Keeping Up Disapperances",
    img: "MNGKQDW.png",
    opts: {
      series: ["pax"]
    }
  },
  {
    name: "Hot to the Touch, Cold on the Inside",
    img: "MNGKQDW.png",
    opts: {
      series: ["pax"]
    }
  },
  {
    name: "Love, Sex, Death",
    img: "MNGKQDW.png",
    opts: {
      series: ["pax"]
    }
  },
  {
    name: "Eternal Summer",
    img: "MNGKQDW.png",
    opts: {
      series: ["pax"]
    }
  },
  {
    name: "Demigods",
    img: "MNGKQDW.png",
    opts: {
      series: ["pax"]
    }
  },
  {
    name: "American Made",
    img: "MNGKQDW.png",
    opts: {
      series: ["pax"]
    }
  },
  {
    name: "Caffine Cold",
    img: "MNGKQDW.png",
    opts: {
      series: ["pax"]
    }
  },
  {
    name: "Save Rock and Roll",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "The Pheonix",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "My Songs Know What You Did In the Dark",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "Alone Together",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "Where Did The Party Go",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "Just One Yesterday",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "The Mighty Fall",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "Miss Missing You",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "Death Valley",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "Young Volcaones",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "Rat A Tat",
    img: "XsjGhie.jpeg",
    opts: {
      series: ["save"]
    }
  },
  {
    name: "Irresistible",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "American Beauty/American Psycho",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "Centuries",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "The Kids Aren't Alright",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "Uma Thurman",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "Jet Pack Blues",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "Novocaine",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "Fourth of July",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "Favorite Record",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "Immortals",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "Twin Skeleton's(Hotel In New York City)",
    img: "Gq8LGGM.png",
    opts: {
      series: ["abap"]
    }
  },
  {
    name: "Stay Frosty Royal Milk Tea",
    img: "fsJ9SvP.png",
    opts: {
      series: ["mania"]
    }
  },
  {
    name: "The Last Of The Real Ones",
    img: "fsJ9SvP.png",
    opts: {
      series: ["mania"]
    }
  },
  {
    name: "HOLD ME TIGHT OR DON'T",
    img: "fsJ9SvP.png",
    opts: {
      series: ["mania"]
    }
  },
  {
    name: "Wilson (Expensive Mistakes)",
    img: "fsJ9SvP.png",
    opts: {
      series: ["mania"]
    }
  },
  {
    name: "Church",
    img: "fsJ9SvP.png",
    opts: {
      series: ["mania"]
    }
  },
  {
    name: "Heaven's Gate",
    img: "fsJ9SvP.png",
    opts: {
      series: ["mania"]
    }
  },
  {
    name: "Champion",
    img: "fsJ9SvP.png",
    opts: {
      series: ["mania"]
    }
  },
  {
    name: "Sunshine Riptide",
    img: "fsJ9SvP.png",
    opts: {
      series: ["mania"]
    }
  },
  {
    name: "Young and Menace",
    img: "fsJ9SvP.png",
    opts: {
      series: ["mania"]
    }
  },
  {
    name: "Bishops Knife Trick",
    img: "fsJ9SvP.png",
    opts: {
      series: ["mania"]
    }
  },
  {
    name: "Lake Effect Kid",
    img: "gn8wuMs.png",
    opts: {
      series: ["lake"]
    }
  },
  {
    name: "City In A Garden",
    img: "yxomhBn.png",
    opts: {
      series: ["lake"]
    }
  },
  {
    name: "Super Fade",
    img: "yxomhBn.png",
    opts: {
      series: ["lake"]
    }
  },
  {
    name: "I've Been Waiting",
    img: "gn8wuMs.png",
    opts: {
      series: ["fob"]
    }
  },
  {
    name: "Summer Days",
    img: "gn8wuMs.png",
    opts: {
      series: ["fob"]
    }
  },
  {
    name: "Dear Future Self (Hands Up)",
    img: "gn8wuMs.png",
    opts: {
      series: ["fob"]
    }
  },
  {
    name: "Bob Dylan",
    img: "gn8wuMs.png",
    opts: {
      series: ["fob"]
    }
  },
  {
    name: "Love From The Other Side",
    img: "FkHpHan.png",
    opts: {
      series: ["stardust"]
    }
  },
  {
    name: "Heartbreak Feels So Good",
    img: "FkHpHan.png",
    opts: {
      series: ["stardust"]
    }
  }
];
